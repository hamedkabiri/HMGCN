...
def node_classification_evaluate(model, feature, A, node_types, file_name, file_type, device, isTest=True):
    """Node classification training process"""




    embeds = model(feature, A, node_types)
    ...
    for _ in range(10):
        ...
        opt = torch.optim.Adam([{'params': model.parameters(), 'lr': 0.05}, {'params': log.parameters()}], lr=0.005,
                               weight_decay=0.0005)
        log.to(device)
    ...
    if isTest:
        print("\t[Classification] Macro-F1: {:.4f} ({:.4f}) | Micro-F1: {:.4f} ({:.4f}) | accuracy: {:.4f} ".format( np.mean(macro_f1s),
                                                                                                np.std(macro_f1s),
                                                                                                np.mean(micro_f1s),
                                                                                                np.std(micro_f1s), np.mean(accs)))
    else:
        return np.mean(accs), np.mean(macro_f1s), np.std(macro_f1s), np.mean(micro_f1s), np.std(micro_f1s)
